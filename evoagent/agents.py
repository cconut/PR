...
整体的agents的流程是：security-agent、reliability-agent、llm-review、动态技能各自报的发现，
经过 critic 质疑、test 复现、synthesizer 合并去重、verifier 终审之后，得到的那一份联合清单。
...
# 在开始review之前，对parsed diff进行分析，判断文件类型、敏感性、风险等级等，生成review plan。
class PlannerAgent:
    name = "planner-agent"
    ...
    def plan(self, parsed: ParsedDiff, specialists: List[Reviewer]) -> ReviewPlan:
        ...
        # 文件超过 10 个 → 大改动，risk_level 标 high。
        return ReviewPlan(
...
# 验证 Bug 真实性，去代码里踩一脚，看看到底能不能复现这个 Bug
class TestAgent:
    name = "test-agent"
