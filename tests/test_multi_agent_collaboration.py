# Added coverage for the expanded multi-agent collaboration protocol.
import os
import tempfile
import unittest
...
from evoagent.runtime import RuntimeBudgetExceeded
...
class MultiAgentCollaborationTests(unittest.TestCase):
    ...
    def test_collaboration_stages_are_compiled_as_a_langgraph(self):
        coordinator = MultiAgentCoordinator([SecurityRuleReviewer()])

        graph = coordinator.graph.get_graph()

        self.assertTrue(type(coordinator.graph).__module__.startswith("langgraph"))
        self.assertTrue({
            "planner", "specialists", "deliberation", "evidence", "verifier", "arbiter",
        }.issubset(graph.nodes))

    def test_langgraph_collaboration_keeps_the_step_budget(self):
        coordinator = MultiAgentCoordinator([SecurityRuleReviewer()])
        coordinator.collaboration_max_steps = 1
